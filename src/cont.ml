...
module Edge = struct
  ...
  let before_display__for_proc2 ?(here = Stdlib.Lexing.dummy_pos) callback graph =
    perform ~here graph (Proc.Edge.before_display ~here callback)
  ;;

  let before_display ?(here = Stdlib.Lexing.dummy_pos) callback graph =
    ignore_t (before_display__for_proc2 ~here callback graph)
  ;;

  let before_display'__for_proc2 ?(here = Stdlib.Lexing.dummy_pos) callback graph =
    perform ~here graph (Proc.Edge.before_display' ~here callback)
  ;;
  ...
  let wait_before_display ?(here = Stdlib.Lexing.dummy_pos) graph =
...
module Dynamic_scope = struct
  ...
  module Bulk_setter = struct
    ...
    let set ?(here = Stdlib.Lexing.dummy_pos) hlist ~inside graph =
      let inside = handle ~here graph ~f:(fun graph -> inside graph) in
...
module Let_syntax = struct
  ...
  module Let_syntax = struct
    ...
    let debug_node ?(here = Stdlib.Lexing.dummy_pos) ~name ~equal ~sexp_of value =
      with_global_graph
