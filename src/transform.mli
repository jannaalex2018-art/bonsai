...
(** Both [For_value.map] and [For_computation.map] involve interacting universally
    quantified functions. Since OCaml does not support first-class universally quantified
    functions, so we have to wrap the function in a single-field record (we have done this
    ...
    - possibly an id representing the variable to which the current node will be bound
      (that is, the current node is either the [from] field of a [subst] or the [inner]
      field of a [store].
    - something called [var_from_parent], which is the uid of any type id contained which
      introduces a value from the immediate parent node. This will be [Some] if the
      immediate parent is a [Subst], [Subst_stateless], or [Store] node, since those each
      introduce a new type id into scope. This parameter can be used to build up a mapping
      from type id to the computation that type id represents.
    - a value passed down from its direct parent. This is not quite the same concept as a
      fold accumulator, although it is similar (you could think of it as the accumulator
      for folding over a particular path through the tree.
    ...
    Note that you could invoke the mapper either before or after transforming the current
    node (these correspond to post- and pre- order traversal). *)
